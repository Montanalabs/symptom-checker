...
Untrusted input is a symptom report; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a symptom report
- **Closed type:** `type Decision = Advise(Level) | Escalate` where `Level` is closed
- **Sink / capability:** `grant advise confidence 70` → `privileged { advise(d) }`
- **Consequence axes:** Trust, Confidence

## Run the demo

```sh
examples/apps/symptom-checker/demo.sh
```
...
- `symptom-checker_safe.wave` — the correct design.
- `symptom-checker_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
